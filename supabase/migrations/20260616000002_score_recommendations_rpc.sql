-- Migration: score_today_recommendations RPC
-- Moves the full recommendation scoring pipeline from TypeScript to PostgreSQL.
-- Single RPC replaces: fetchAllGames (46 queries) + mapGameRowToSeedGame (44k rows) + buildTodayModel (40k scores in JS)
...
as $$
  select case p_tag
    when 'story_rich' then 3.0
    when 'lore_heavy' then 2.5
    when 'minimalist_story' then 1.0
    when 'branching_narrative' then 3.0
    when 'text_based' then 2.0
    when 'souls_like' then 4.0
    when 'stealth' then 3.0
    when 'puzzle' then 2.5
    when 'rhythm' then 2.5
    when 'tactical' then 3.0
    when 'deck_building' then 3.0
    when 'immersive_sim' then 3.5
    when 'survival' then 2.5
    when 'open_world' then 2.0
    when 'linear' then 1.5
    when 'hub_based' then 1.5
    when 'roguelike' then 2.5
    when 'metroidvania' then 3.0
    when 'sandbox' then 2.0
    when 'demanding' then 3.0
    when 'unforgiving' then 3.5
    when 'chill' then 2.5
    when 'accessible' then 2.0
    when 'short_sessions' then 1.0
    when 'long_sessions' then 1.5
    when 'pick_up_and_play' then 2.0
    when 'dark' then 1.5
    when 'lighthearted' then 1.5
    when 'horror' then 2.5
    when 'cozy' then 2.0
    else 2.0
  end;
$$;

create or replace function games_library.weighted_cosine_similarity(
  p_game_tags text[],
...
as $$
declare
  v_dot_product double precision := 0;
  v_norm_game double precision := 0;
  v_norm_profile double precision := 0;
  v_tag text;
  v_weight double precision;
  v_profile_count double precision;
  v_key text;
begin
  if p_game_tags is null or array_length(p_game_tags, 1) is null then
    return 0;
  end if;
  if p_profile_tags is null or p_profile_tags = '{}'::jsonb then
    return 0;
  end if;

  foreach v_tag in array p_game_tags
  loop
    v_weight := games_library.tag_weight(v_tag);
    v_norm_game := v_norm_game + (v_weight * v_weight);

    v_profile_count := coalesce((p_profile_tags ->> v_tag)::double precision, 0);
    if v_profile_count > 0 then
      v_dot_product := v_dot_product + (v_weight * v_profile_count * v_weight);
    end if;
  end loop;

  for v_key in select jsonb_object_keys(p_profile_tags)
  loop
    v_weight := games_library.tag_weight(v_key);
    v_profile_count := (p_profile_tags ->> v_key)::double precision;
    v_norm_profile := v_norm_profile + ((v_profile_count * v_weight) * (v_profile_count * v_weight));
  end loop;

  if v_norm_game = 0 or v_norm_profile = 0 then
    return 0;
  end if;

  return v_dot_product / (sqrt(v_norm_game + 15.0) * sqrt(v_norm_profile));
end;
$$;

create or replace function games_library.confidence_label(p_rated_count int)
returns text
language sql
immutable
as $$
...
create or replace function games_library.format_trait(p_value text)
returns text
language sql
immutable
as $$
...
-- ============================================================
-- Step 2: Main RPC — full recommendation pipeline
-- ============================================================
...
begin
  ...
  with
  -- Flatten game states: navigate JSONB object -> rows
  ...
  -- Score all scorable games with tag overlap
  scored_games as (
    select
      g.game_id,
      g.title,
    ...
    where (
      -- Pre-filter by tag overlap (Phase 1 optimization)
  ...
  -- Compute affinity + risk (matching TypeScript scoreSeedGame exactly)
  computed as (
    select
      *,
...
grant execute on function games_library.tag_weight to anon, authenticated, service_role;
grant execute on function games_library.weighted_cosine_similarity to anon, authenticated, service_role;
grant execute on function games_library.confidence_label to anon, authenticated, service_role;
grant execute on function games_library.format_trait to anon, authenticated, service_role;
...
-- drop function if exists games_library.format_trait;
-- drop function if exists games_library.confidence_label;
-- drop function if exists games_library.weighted_cosine_similarity;
-- drop function if exists games_library.tag_weight;
