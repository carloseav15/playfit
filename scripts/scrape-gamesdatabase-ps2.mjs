...
const URLS_FILE = "/tmp/gdb-ps2-urls.txt";
const DELAY_MS = 350;
const OUT_FILE = new URL("../reports/gamesdatabase-ps2.json", import.meta.url);
...
function parseDetailPage(html, slug) {
  ...
  const titleMatch = $("title").text().match(/^\s*(.+?)\s*-\s*Sony Playstation 2\s*-\s*Games Database/);
