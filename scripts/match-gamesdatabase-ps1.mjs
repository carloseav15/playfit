...
const SCRAPE_FILE = new URL("../reports/gamesdatabase-ps1.json", import.meta.url);
const PS1_TSV = "/tmp/ps1_games.tsv";
const OUT_FILE = new URL("../reports/gamesdatabase-ps1-apply.json", import.meta.url);
...
function main() {
  ...
  const ps1Rows = readFileSync(PS1_TSV, "utf8")
    .split("\n")
    .filter(Boolean)
    .map((line) => {
  ...
  console.log(`With developer to apply: ${results.filter((r) => r.apply_developer).length}`);
  console.log(`With publisher to apply: ${results.filter((r) => r.apply_publisher).length}`);
