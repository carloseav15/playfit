...
const APPLY_FILE = new URL("../reports/gamesdatabase-ps1-apply.json", import.meta.url);
const COVERS_DIR = new URL("../apps/web/public/covers/games/", import.meta.url);
const OUT_FILE = new URL("../reports/gamesdatabase-ps1-covers-downloaded.json", import.meta.url);
const DELAY_MS = 350;
...
function extractBigImagePath(html) {
  ...
  for (const img of imgs) {
    ...
    const match = src.match(/\/Media\/SYSTEM\/Sony_Playstation\/Box\/big\/(.+)\.jpg/i);
...
async function main() {
  ...
  for (const t of remaining) {
    ...
    if (publisherSlug && year) {
      urlsToTry.push(
        `https://www.gamesdatabase.org/media/sony-playstation/artwork-box/${year}/${publisherSlug}/${t.slug}`,
      );
    }
    if (year) {
      urlsToTry.push(
        `https://www.gamesdatabase.org/media/sony-playstation/artwork-box/${year}/${t.slug}`,
      );
    }
