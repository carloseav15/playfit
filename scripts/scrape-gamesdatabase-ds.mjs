...
const URLS_FILE = "/tmp/gdb_ds_urls.txt";
const DELAY_MS = 350;
const OUT_FILE = new URL("../reports/gamesdatabase-ds.json", import.meta.url);
...
function parseDetailPage(html, slug) {
  ...
  const titleMatch = $("title").text().match(/^\s*(.+?)\s*-\s*Nintendo DS\s*-\s*Games Database/);
