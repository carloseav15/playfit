...
const SCRAPE_FILE = new URL("../reports/gamesdatabase-snes.json", import.meta.url);
const SNES_TSV = "/tmp/snes_games.tsv";
const OUT_FILE = new URL("../reports/gamesdatabase-snes-apply.json", import.meta.url);
...
function main() {
  ...
  const snesRows = readFileSync(SNES_TSV, "utf8")
    .split("\n")
    .filter(Boolean)
    .map((line) => {
