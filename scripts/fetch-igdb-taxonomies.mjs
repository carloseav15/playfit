// Fetches game_modes, themes and player_perspectives ids from IGDB for games
// already linked via games_library.game_external_ids (provider='igdb').
// These are new taxonomies for Playfit (games_library.game_modes/themes/
// perspectives, seeded by migration 20260704_seed_igdb_taxonomy_lookups).
//
// Output: reports/igdb-taxonomies.ndjson, one record per igdb_id:
//   { igdb_id, game_modes: [igdb_id,...], themes: [...], player_perspectives: [...] }
import { createClient } from "@supabase/supabase-js";
import { writeFileSync, appendFileSync } from "node:fs";

const OUT_FILE = new URL("../reports/igdb-taxonomies.ndjson", import.meta.url);
...
async function main() {
  ...
  for (let i = 0; i < igdbIds.length; i += BATCH_SIZE) {
    ...
    const res = await fetch("https://api.igdb.com/v4/games", {
      ...
      body: `fields id,game_modes,themes,player_perspectives;
             where id = (${batch.join(",")});
             limit ${BATCH_SIZE};`,
    });
    ...
    const lines = rows
      .filter(
        (g) =>
          (g.game_modes ?? []).length > 0 ||
          (g.themes ?? []).length > 0 ||
          (g.player_perspectives ?? []).length > 0,
      )
      .map((g) =>
        JSON.stringify({
          igdb_id: g.id,
    ...
    total += rows.length;
    if (total % 5000 < BATCH_SIZE) console.log(`taxonomies: ${total}/${igdbIds.length} fetched`);
