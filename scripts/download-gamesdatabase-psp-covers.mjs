...
const APPLY_FILE = new URL("../reports/gamesdatabase-psp-apply.json", import.meta.url);
const COVERS_DIR = new URL("../apps/web/public/covers/games/", import.meta.url);
const OUT_FILE = new URL("../reports/gamesdatabase-psp-covers-downloaded.json", import.meta.url);
const DELAY_MS = 350;
...
function extractBigImagePath(html) {
  ...
  for (const img of imgs) {
    ...
    const match = src.match(/\/Media\/SYSTEM\/Sony_PSP\/Box\/big\/(.+)\.jpg/i);
...
async function main() {
  ...
  for (const t of remaining) {
    ...
    if (publisherSlug && year) {
      urlsToTry.push(
        `https://www.gamesdatabase.org/media/sony-psp/artwork-box/${year}/${publisherSlug}/${t.slug}`,
      );
    }
    if (year) {
      urlsToTry.push(
        `https://www.gamesdatabase.org/media/sony-psp/artwork-box/${year}/${t.slug}`,
      );
    }
    ...
    try {
      ...
      ok += 1;
      console.log(`  OK ${t.game_id} (${buf.length} bytes) - ${ok}/${553}`);
    } catch (e) {
