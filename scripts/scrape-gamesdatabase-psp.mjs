// Scrapes gamesdatabase.org's Sony PSP catalog (robots.txt explicitly allows
// crawling: "Allow: /"). Pulls the game slug list from the site's own
// sitemap (type=games&part=9, where all sony-psp URLs live), then visits
// each game detail page and extracts only structured, factual fields
// (Publisher, Developer, Year, Category) -- the free-text marketing blurb on
// each page is intentionally NOT captured or stored.
import { load } from "cheerio";
import { readFileSync, writeFileSync } from "node:fs";

const URLS_FILE = "/private/tmp/claude-501/-Users-carancibia-Projects-playfit/1d7a580e-06e2-486a-bb08-4bee589bdee0/scratchpad/gdb_psp_urls.txt";
const DELAY_MS = 350;
const OUT_FILE = new URL("../reports/gamesdatabase-psp.json", import.meta.url);
...
function parseDetailPage(html, slug) {
  ...
  const titleMatch = $("title").text().match(/^\s*(.+?)\s*-\s*Sony PSP\s*-\s*Games Database/);
