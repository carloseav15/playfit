...
const URLS_FILE = "/tmp/gdb_saturn_urls.txt";
const DELAY_MS = 350;
const OUT_FILE = new URL("../reports/gamesdatabase-saturn.json", import.meta.url);
...
function parseDetailPage(html, slug) {
  ...
  const titleMatch = $("title").text().match(/^\s*(.+?)\s*-\s*Sega Saturn\s*-\s*Games Database/);
...
async function main() {
  ...
  const remaining = urls.filter((url) => !doneSlugs.has(url.split("/").pop()));
