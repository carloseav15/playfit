...
const APPLY_FILE = new URL("../reports/gamesdatabase-gba-apply.json", import.meta.url);
const COVERS_DIR = new URL("../apps/web/public/covers/games/", import.meta.url);
const OUT_FILE = new URL("../reports/gamesdatabase-gba-covers-downloaded.json", import.meta.url);
const DELAY_MS = 350;
...
function extractBigImagePath(html) {
  ...
  for (const img of imgs) {
    ...
    const match = src.match(/\/Media\/SYSTEM\/Nintendo_Game_Boy_Advance\/Box\/big\/(.+)\.jpg/i);
...
async function main() {
  ...
  for (const t of remaining) {
    ...
    if (publisherSlug && year) {
      urlsToTry.push(
        `https://www.gamesdatabase.org/media/nintendo-game-boy-advance/artwork-box/${year}/${publisherSlug}/${t.slug}`,
      );
    }
    if (year) {
      urlsToTry.push(
        `https://www.gamesdatabase.org/media/nintendo-game-boy-advance/artwork-box/${year}/${t.slug}`,
      );
    }
