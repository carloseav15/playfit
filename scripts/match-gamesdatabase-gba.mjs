...
const SCRAPE_FILE = new URL("../reports/gamesdatabase-gba.json", import.meta.url);
const GBA_TSV = "/tmp/gba_games.tsv";
const OUT_FILE = new URL("../reports/gamesdatabase-gba-apply.json", import.meta.url);
...
function main() {
  ...
  function parsePgTsvValue(v) {
    return v === "\\N" || v === null || v === undefined ? null : v;
  }
  const gbaRows = readFileSync(GBA_TSV, "utf8")
    .split("\n")
    .filter(Boolean)
    .map((line) => {
      ...
      return {
        game_id,
        title,
        genre_id: parsePgTsvValue(genre_id),
        cover_url: parsePgTsvValue(cover_url),
        release_year: parsePgTsvValue(release_year) ? Number(release_year) : null,
      };
    });

  const byNormTitle = new Map();
  for (const row of gbaRows) {
  ...
  console.log(`Scraped (no errors): ${scraped.filter(r => !r.error).length}`);
  console.log(`Matched: ${matched}`);
  console.log(`Ambiguous (multiple games share title): ${ambiguous}`);
  console.log(`Excluded (year gap > 3): ${yearExcluded}`);
  console.log(`Unmatched: ${scraped.filter(r => !r.error).length - matched - ambiguous - yearExcluded}`);
