...
const SCRAPE_FILE = new URL("../reports/gamesdatabase-saturn.json", import.meta.url);
const SATURN_TSV = "/tmp/saturn_games.tsv";
const OUT_FILE = new URL("../reports/gamesdatabase-saturn-apply.json", import.meta.url);
...
function main() {
  ...
  const saturnRows = readFileSync(SATURN_TSV, "utf8")
    .split("\n")
    .filter(Boolean)
    .map((line) => {
