...
const SCRAPE_FILE = new URL("../reports/gamesdatabase-ds.json", import.meta.url);
const DS_TSV = "/tmp/ds_games.tsv";
const OUT_FILE = new URL("../reports/gamesdatabase-ds-apply.json", import.meta.url);
...
function main() {
  ...
  const dsRows = readFileSync(DS_TSV, "utf8")
    .split("\n")
    .filter(Boolean)
    .map((line) => {
