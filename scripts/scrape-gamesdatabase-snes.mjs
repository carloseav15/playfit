...
const URLS_FILE = "/tmp/gdb_snes_urls.txt";
const DELAY_MS = 350;
const OUT_FILE = new URL("../reports/gamesdatabase-snes.json", import.meta.url);
...
function parseDetailPage(html, slug) {
  ...
  const titleMatch = $("title").text().match(/^\s*(.+?)\s*-\s*Nintendo SNES\s*-\s*Games Database/);
