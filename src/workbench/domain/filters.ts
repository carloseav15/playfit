...
export function filterAndSortRecords(
  ...
  const searchedRecords = query ? searchIndex.search(query).map((result) => result.item) : records;

  const filteredRecords = searchedRecords.filter((record) => {
    const matchesStatus = filters.status === "all" || record.status === filters.status;
    const matchesGenre = filters.genre === "all" || record.primaryGenre === filters.genre;
...
export function uniqueStatuses(records: GameRecord[]) {
  return [...new Set(records.map((record) => record.status).filter(Boolean))].sort((left, right) =>
    left.localeCompare(right),
  );
}

export function uniqueGenres(records: GameRecord[]) {
  return [...new Set(records.map((record) => record.primaryGenre).filter(Boolean))].sort(
